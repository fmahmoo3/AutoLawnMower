...
uint32_t leftMotorDutyCycle = 0;
uint32_t rightMotorDutyCycle = 0;
...
void increasePWM(){
    if(leftMotorDutyCycle*100/MOTORMAXPERIOD < 99){
    ...
    sprintf(str,"\n\rRobot PWM set to %d\n\r",rightMotorDutyCycle*100/MOTORMAXPERIOD);
    putString(str);
}

void decreasePWM(){
    if((leftMotorDutyCycle*100/MOTORMAXPERIOD) > 4){
        changeDutyCyclePercent(leftMotorDutyCycle*100/MOTORMAXPERIOD-1, left);
        changeDutyCyclePercent(rightMotorDutyCycle*100/MOTORMAXPERIOD-1, right);
    }

    if(leftMotorDutyCycle*100/MOTORMAXPERIOD < 3){
        stop();
    }
    else{
        start();
...
void lowSpeed(){
    ...
    changeDutyCyclePercent(35, left);
    changeDutyCyclePercent(34, right);

    putString("Robot PWM set to 34");
}
...
void setRightMotorSpeed(int millisec){
    rightMotorSpeed = millisec;
}
